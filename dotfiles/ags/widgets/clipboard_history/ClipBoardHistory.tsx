...
function EmptyClipBoard() {
  return (
    <box
      ...
      hexpand
    >
      <image iconName={"clipman-symbolic"} iconSize={Gtk.IconSize.LARGE} />
    ...
    <window
      ...
      setup={(self) => {
        self.connect("notify::visible", () => {
          if (self.visible) {
            execAsync(["bash", "-c", "cliphist --preview-width 40 list"])
              .then((output) => {
                if (output == "") {
                ...
                clipboardHistory.set(
                  output.split("\n").map((entry) => {
                    return {
                      text: entry.split("\t")[1],
                      clipHistIndex: parseInt(entry.split("\t")[0]),
                ...
                <box vexpand hexpand vertical spacing={10}>
                  {clipboardHistory.map((entry) => {
                    return (
                      <ClipBoardEntry
                        text={entry.text}
                        clipHistIndex={entry.clipHistIndex}
                      />
                    );
                  })}
                </box>
