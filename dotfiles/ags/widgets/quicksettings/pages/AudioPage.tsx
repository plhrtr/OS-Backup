                ...
                setup={(self) => {
                  ...
                  const speakerList = speakers.map((speaker) =>
                    speaker.description.length < 30
                      ? speaker.description
                      : speaker.description.slice(0, 30) + "...",
                  );
                  self.model = Gtk.StringList.new(speakerList);
                  self.connect("notify::selected-item", () => {
                    const selectedIndex = self.selected;
                    const selectedSpeaker = speakers[selectedIndex];
                    if (selectedSpeaker) {
                      selectedSpeaker.set_is_default(true);
                    }
                  });
                }}
              />
