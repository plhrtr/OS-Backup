    ...
    >
      <box vertical spacing={4} hexpand cssClasses={["bluetooth-page"]}>
        <box cssClasses={["qs-page-content-header"]}>
          <label label={"Use bluetooth"} halign={Gtk.Align.START} hexpand />
          <switch
        ...
        {bind(bluetooth, "isPowered").as((isPowered) => {
          if (isPowered) {
            return (
              <box vertical spacing={4}>
                <box cssClasses={["qs-page-content"]} vertical spacing={10}>
                  <label
                ...
                >
                  <label
                    ...
                    cssClasses={["headline"]}
                  />
                  {bind(previousConnectedDevices).as((devices) => {
                    const seenDevices = new Set();
                    return devices.length > 0 ? (
                      devices
                        .filter((device) => {
                          if (seenDevices.has(device) || device.connected) {
                            return false;
                          }
                      ...
                      <box
                        ...
                        cssClasses={["no-entry"]}
                      >
                        No previously connected devices
                      </box>
                    );
                  })}
                </box>
          ...
          return (
            <box cssClasses={["qs-page-content-footer", "no-entry"]}>
              <label
