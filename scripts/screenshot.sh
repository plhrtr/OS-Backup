...
if [ -f "${save_dir}/${save_file}" ]; then
	ret_val=$(notify-send --action "action1=Open in file explorer" --action "action2=Delete" -a "Screenshot" -i "image-x-generic-symbolic" "Screenshot saved in '${save_dir}'")
	case $ret_val in
	"action1") xdg-open "$save_dir" ;;
	"action2") rm "${save_dir}/${save_file}" ;;
	esac
fi
fi
